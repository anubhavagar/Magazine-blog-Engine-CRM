...
//     $("#brknewsticker").ticker({ effect: "slideDown"});
     $("#brknewsticker").ticker({
...
$(document).ready(function () {
    //    $('#breaking_news_slideshow').cycle({
    //    fx:     'scrollUp',
    //    timeout: 6000,
    //    delay:  -2000
    //	});

    if ($("#textsizer-classnames").length > 0) {
    ...
    //var Data = JSON.stringify({ category: 'krantikari-kavitayen' });
    $.ajax({
        type: "POST",
        url: "/Home/GetDataByCategory",
        //data: Data,
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (response) {
            isProcessing = false;
            $.each(response.Articles_desh, function (index, value) {
                $('#desh_list ul').append('<li><a href="/Post/' + response.Articles_desh[index].ID + '/' + response.Articles_desh[index].Title_SEO + '" rel="bookmark" title="' + response.Articles_desh[index].Title + '">' + response.Articles_desh[index].Title + '</a></li>');
            });

            $.each(response.Articles_kavitayen, function (index, value) {
                $('#kavitayen_list ul').append('<li><a href="/Post/' + response.Articles_kavitayen[index].ID + '/' + response.Articles_kavitayen[index].Title_SEO + '" rel="bookmark" title="' + response.Articles_kavitayen[index].Title + '">' + response.Articles_kavitayen[index].Title + '</a></li>');
            });

            $.each(response.Articles_videsh, function (index, value) {
                $('#videsh_list ul').append('<li><a href="/Post/' + response.Articles_videsh[index].ID + '/' + response.Articles_videsh[index].Title_SEO + '" rel="bookmark" title="' + response.Articles_videsh[index].Title + '">' + response.Articles_videsh[index].Title + '</a></li>');
            });
        },
        error: function (result) { isProcessing = false; return false; }
